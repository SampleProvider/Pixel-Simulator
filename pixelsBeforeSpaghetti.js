    ...
    push: while (queue.length > 0) {
        ...
        for (x2 = x1; x2 >= 0; x2--) {
            ...
            if (id == COLLAPSABLE && x2 != x1) {
                ...
                if (canCollapse) {
                    collapsableXPos = x2;
                    // console.log(failedPushPixels[x2 + y1 * gridWidth])
                    if (failedPushPixels[x2 + y1 * gridWidth] == tick) {
                        break;
                    }
                        ...
                        //     break;
                        // }
                        if (pushLeftCheck(x2, y1, false)[0]) {
                            break;
                        }
                    }
                }
...
function pushLeft(x, y, pushPistons) {
    let [worked, pushPixels, hasCollapsable] = pushLeftCheck(x, y);
    let queue = [x + y * gridWidth + gridWidth * gridHeight];
    // let worked = true;
    // let hasCollapsable = false;
    push: while (queue.length > 0) {
    ...
    if (worked) {
        ...
        if (hasCollapsable) {
            pushPixels = [];
            queue = [x + y * gridWidth + gridWidth * gridHeight];
            worked = true;
            push: while (queue.length > 0) {
                let index = queue.pop();
                let stronglyConnected = index > gridWidth * gridHeight;
...
//     return false;
// };
function pushLeftCheck(x, y, stronglyConnected) {
    if (x == pushX && y == pushY) {
        return true;
    }
