import { grid, gridWidth, gridHeight, gridStride, chunks, nextChunks, chunkWidth, chunkHeight, chunkXAmount, chunkYAmount, chunkStride, tick, setBrushPixel, showTooltip, hideTooltip, moveTooltip } from "./game.js";
...
let failedPushPixels2 = new Int32Array();
let pushStack = [];
...
function pushLeft(x, y) {
    ...
    // alert(x + " " + y)
    // return;
    let worked = pushLeftCheck(x, y, true, false);
    // let worked = true;
    // pushStack = [[x, y, true, false]];
    // while (pushStack.length > 0 && (worked = pushLeftCheck(...pushStack.pop()))) {

    // }
    // pushStack = [];
    // let failedPushPixels;
    // if (failedPushPixels[x + y * gridWidth] == tick) {
    //     pushPixels = [];
    //     return false;
    // }
    if (!worked) {
        // failedPushPixels = pushPixels;
        // for (let y = 0; y < gridHeight; y++) {
        //     for (let x = 0; x < gridWidth; x++) {
        //         if (failedPushPixels2[x + y * gridWidth] == pushNumber + 1) {
        //             failedPushPixels[x + y * gridWidth] = tick;
        //         }
        //     }
        // }
        ...
        pushPixels = [];
        worked = pushLeftCheck(x, y, true, true);
        // worked = true;
        // pushStack = [[x, y, true, true]];
        // while (pushStack.length > 0 && (worked = pushLeftCheck(...pushStack.pop()))) {
    ...
    // console.log(grid[(x + y * gridWidth) * gridStride + UPDATED], tick)
    if (worked) {
        // alert("hi")
        for (let i in pushPixels) {
            for (let j in pushPixels[i]) {
                ...
                move(j, i, j - 1, i);
                // console.log(grid[index + UPDATED], tick)
                // alert(j + " " + i)
                // let pixel = grid[i][j];
                // grid[i][j - 1] = pixel;
                // grid[i][j] = null;
                // pixel.gridX = j - 1;
                // pixel.speedX = -PIXEL_SPEED;
                // pixel.movingX = true;
                // if (pixel.updated && pixel.movedX != Math.sign(pixel.speedX) && pixel.id != PISTON_LEFT && pixel.id != STICKY_PISTON_LEFT && pixel.id != ANTISTICKY_PISTON_LEFT) {
                //     move(pixel, 0, 0, true, true, function(pixel1, moved) {
                //         if (pixel1 == null) {
                //             return true;
                //         }
                //         pixel.x = pixel.gridX * gridSize;
                //         pixel.speedX = 0;
                //         return false;
                //     }, function(pixel1, moved) {
                //         pixel.y = pixel.gridY * gridSize;
                //         pixel.speedY = 0;
                //         return false;
                //     });
                //     // stationary(pixel);
...
function pushLeftCheck(x, y, strong, collapsable) {
    if (x == pushX && y == pushY) {
        return true;
    }
    function pushFail(x1) {
        for (let x2 = x; x2 >= x1; x2--) {
            // pushPixels[y][x2] = strong ? 2 : 1;
            pushPixels[y][x2] = 2;
            // failedPushPixels[y][x2] = strong ? 1 : 0;
            // failedPushPixels[x2 + y * gridWidth + (collapsable ? (gridWidth * gridHeight) : 0)] = tick;
            // failedPushPixels2[x2 + y * gridWidth + (collapsable ? (gridWidth * gridHeight) : 0)] = pushNumber + (strong ? 1 : 0);
            // if (strong && collapsable) {
            //     let index = (x2 + y * gridWidth) * gridStride;
            //     grid[index + UPDATED] = tick;
            // }
            // failedPushPixels[x2 + y * gridWidth + (collapsable ? 0 : 0)] = tick;
            // failedPushPixels[x2 + y * gridWidth] = tick;
        }
    };
    let xPos = -1;
    ...
    for (let x1 = x; x1 >= 0; x1--) {
        ...
        let id = grid[index + ID];
        // if (grid[id] == AIR || (collapsable && grid[id] == COLLAPSIBLE)) {
        if (id == AIR) {
            xPos = x1;
            break;
        }
        if (collapsable && id == COLLAPSABLE) {
            collapsableXPos = x1;
        }
        if (failedPushPixels[x1 + y * gridWidth + (collapsable ? (gridWidth * gridHeight) : 0)] == tick) {
            // if (failedPushPixels[x1 + y * gridWidth] == tick) {
            //     // pushFail(x1);
        ...
        // if (grid[y][x1] == null || (collapsable && grid[y][x1].id == COLLAPSABLE_BOX)) {
        //     index = i;
        //     break;
        // }
        ...
        if (pushPixels[y][x1] == null) {
            // if (grid[y][x1].speedX == -PIXEL_SPEED) {
            //     return false;
            // }
            if (!pixels[id].pushable) {
                pushFail(x1);
            ...
            // if (grid[y][x1].id == PISTON_RIGHT || grid[y][x1].id == STICKY_PISTON_RIGHT || grid[y][x1].id == ANTISTICKY_PISTON_RIGHT || grid[y][x1].id == SLIDER_VERTICAL) {
            //     return false;
            // }
        }
        else {
            // if (pushPixels[y][x1] != 2 && pushPixels[y][x1] != 1) {
            //     pushFail(x1);
            //     return false;
            // }
            xPos = x1;
    ...
    // for (let x1 = x; i < index; i++) {
    for (let x1 = x; x1 > xPos; x1--) {
        pushPixels[y][x1] = strong ? 2 : 1;
        if (failedPushPixels[x1 + y * gridWidth + (collapsable ? (gridWidth * gridHeight) : 0)] == tick) {
            console.log("ee")
        }
        // failedPushPixels2[x1 + y * gridWidth + (collapsable ? (gridWidth * gridHeight) : 0)] = pushNumber + (strong ? 1 : 0);
        // let index = (x1 + y * gridWidth) * gridStride;
        // grid[index + UPDATED] = tick;
    }
    ...
    for (let x1 = x; x1 > xPos; x1--) {
        ...
        if (pixels[id].sticky) {
            if (y > 0 && !pixels[grid[(x1 + (y - 1) * gridWidth) * gridStride + ID]].antiSticky) {
                // if (grid[(x1 + (y - 1) * gridWidth) * gridStride + ID] == PISTON_LEFT && grid[(x1 + (y - 1) * gridWidth) * gridStride + UPDATED] == tick) {
                //     continue;
                // }
                if (pushPixels[y - 1] == null || pushPixels[y - 1][x1] == null) {
                    let nextStrong = strong && pixels[grid[(x1 + (y - 1) * gridWidth) * gridStride + ID]].sticky;
                    // pushStack.push([x1, y - 1, nextStrong, collapsable]);
                    if (!pushLeftCheck(x1, y - 1, nextStrong, collapsable)) {
                        // for (let x2 = x; x2 >= x1; x2--) {
                        //     pushPixels[y][x2] = nextStrong ? 1 : 0;
                        // }
                        pushFail(x1);
                        return false;
                    }
            ...
            if (x1 == x) {
                if (x < gridHeight - 1 && grid[(x1 + 1 + y * gridWidth) * gridStride + ID] != AIR && !pixels[grid[(x1 + 1 + y * gridWidth) * gridStride + ID]].antiSticky) {
                    // if (grid[(x1 + 1 + y * gridWidth) * gridStride + ID] == PISTON_LEFT && grid[(x1 + 1 + y * gridWidth) * gridStride + UPDATED] == tick) {
                    //     continue;
                    // }
                    if (pushPixels[y] == null || pushPixels[y][x + 1] == null) {
                        let nextStrong = strong && pixels[grid[(x1 + 1 + y * gridWidth) * gridStride + ID]].sticky;
                        // pushStack.push([x + 1, y, nextStrong, collapsable]);
                        if (!pushLeftCheck(x + 1, y, strong && pixels[grid[(x1 + 1 + y * gridWidth) * gridStride + ID]].sticky, collapsable)) {
                            // for (let x2 = x; x2 >= x1; x2--) {
                            //     pushPixels[y][x2] = nextStrong ? 1 : 0;
                            // }
                            pushFail(x1);
        ...
        // if (PIXELS[grid[y][x1].id].sticky) {
        //     if (y > 0 && grid[y - 1][x1] != null && !PIXELS[grid[y - 1][x1].id].antiSticky) {
        //         if (pushPixels[y - 1] == null || pushPixels[y - 1][x1] == null) {
        //             if (!pushLeftCheck(x1, y - 1, collapsable)) {
        //                 for (let j = 0; j <= i; j++) {
        //                     pushPixels[y][x - j] = false;
        //                 }
        //                 return false;
        //             }
        //         }
        //     }
        //     if (y < gridHeight - 1 && grid[y + 1][x1] != null && !PIXELS[grid[y + 1][x1].id].antiSticky) {
        //         if (pushPixels[y + 1] == null || pushPixels[y + 1][x1] == null) {
        //             if (!pushLeftCheck(x1, y + 1, collapsable)) {
        //                 for (let j = 0; j <= i; j++) {
        //                     pushPixels[y][x - j] = false;
        ...
        //     }
        //     if (i == 0) {
        //         if (x < gridHeight - 1 && grid[y][x + 1] != null && !PIXELS[grid[y][x + 1].id].antiSticky) {
        //             if (pushPixels[y] == null || pushPixels[y][x + 1] == null) {
        //                 if (!pushLeftCheck(x + 1, y, collapsable)) {
        //                     for (let j = 0; j <= i; j++) {
        //                         pushPixels[y][x - j] = false;
        //                     }
        //                     return false;
        ...
        //     }
        // }
        // else if (PIXELS[grid[y][x1].id].antiSticky) {
        //     if (y > 0 && grid[y - 1][x1] != null && !PIXELS[grid[y - 1][x1].id].sticky) {
        //         if (pushPixels[y - 1] == null || pushPixels[y - 1][x1] == null) {
        //             pushLeftCheck(x1, y - 1, collapsable);
        //         }
        //     }
        //     if (y < gridHeight - 1 && grid[y + 1][x1] != null && !PIXELS[grid[y + 1][x1].id].sticky) {
        //         if (pushPixels[y + 1] == null || pushPixels[y + 1][x1] == null) {
        //             pushLeftCheck(x1, y + 1, collapsable);
        //         }
        //     }
        //     if (i == 0) {
        //         if (x < gridHeight - 1 && grid[y][x + 1] != null && !PIXELS[grid[y][x + 1].id].sticky) {
        //             if (pushPixels[y] == null || pushPixels[y][x + 1] == null) {
        //                 pushLeftCheck(x + 1, y, collapsable);
        //             }
        //         }
        //     }
        // }
