...
let workedPushPixelsArray = [new Int32Array(), new Int32Array(), new Int32Array(), new Int32Array(), new Int32Array(), new Int32Array()];
let failedPushPixelsArray = [new Int32Array(), new Int32Array(), new Int32Array(), new Int32Array(), new Int32Array(), new Int32Array()];
...
function setPushPixels(canCollapse) {
    workedPushPixels = workedPushPixelsArray[canCollapse ? 3 : 0 + pushStrength];
    failedPushPixels = failedPushPixelsArray[canCollapse ? 3 : 0 + pushStrength];
};

function pushLeft(x, y, selfX, selfY, strength) {
    pushStrength = strength;
    let [worked, pushPixels, hasCollapsable] = pushLeftCheck(x, y, selfX, selfY, 0);
    ...
    if (worked) {
        ...
        if (pushedSelf) {
            ...
            //         addPixel(Number(j), Number(i), SAND);
            //     }
            // }
        }
        else {
        ...
        if (hasCollapsable) {
            [worked, pushPixels, hasCollapsable] = pushLeftCheck(x, y, selfX, selfY, 1);
            pushedSelf = false;
            if (worked && pushPixels[selfY] != null && pushPixels[selfY][selfX] != null) {
                pushedSelf = true;
                worked = false;
...
function pushLeftCheck(x, y, selfX, selfY, canCollapse) {
    setPushPixels(canCollapse);
    let pushPixels = [];
        ...
        for (x2 = x1; x2 >= 0; x2--) {
            ...
            if (id == AIR) {
                ...
                if (pushStrength == 0 && x2 != 0) {
                    ...
                    if (id1 == PUSHER_RIGHT && !isDeactivated(x2 - 1, y1)) {
                        pushFail(x2);
                        break push;
                    }
                }
            ...
            if (id == COLLAPSABLE && (canCollapse != 2 || (x2 != collapseX && y2 != collapseY))) {
                hasCollapsable = true;
                if (canCollapse != 0) {
                    if (canCollapse == 2) {
                        xPos = x2;
                    ...
                    else if (workedPushPixels[x2 + y1 * gridWidth] != tick) {
                    // else if (workedPushPixels[x2 + y1 * gridWidth + gridWidth * gridHeight] != tick) {
                        ...
                        collapseX = x2;
                        collapseY = y1;
                        let [worked1, pushPixels1] = pushLeftCheck(x2, y1, selfX, selfY, 2);
                        let pushedSelf = false;
                        if (worked1 && pushPixels1[selfY] != null && pushPixels1[selfY][selfX] != null) {
                            pushedSelf = true;
                            worked1 = false;
                        }
                        if (worked1) {
            ...
            //     if (workedPushPixels[x2 + y1 * gridWidth] == tick) {
            //         if (x2 != x1) {
            //             pushFail(x2 + 1);
            //             modal("Push error", "Tried to push already pushed pixel left", "error");
            //             break push;
            //         }
            //         continue push;
            //     }
        ...
        for (let x2 = x1; x2 > xPos; x2--) {
            ...
            if (pixels[id].sticky) {
                if (y1 > 0) {
                    let slimeX = x2;
                    let slimeY = y1 - 1;
                    if (pixels[grid[(slimeX + slimeY * gridWidth) * gridStride + ID]].stickableDown && (pushPixels[slimeY] == null || pushPixels[slimeY][slimeX] == null)) {
                ...
                if (y1 < gridHeight - 1) {
                    let slimeX = x2;
                    let slimeY = y1 + 1;
                    if (pixels[grid[(slimeX + slimeY * gridWidth) * gridStride + ID]].stickableUp && (pushPixels[slimeY] == null || pushPixels[slimeY][slimeX] == null)) {
                        queue.push(slimeX + slimeY * gridWidth + (pixels[grid[(slimeX + slimeY * gridWidth) * gridStride + ID]].sticky && stronglyConnected ? (gridWidth * gridHeight) : 0));
                    }
                }
                if (x2 == x1) {
                    if (x2 < gridWidth - 1) {
                        let slimeX = x2 + 1;
                        let slimeY = y1;
                        if (pixels[grid[(slimeX + slimeY * gridWidth) * gridStride + ID]].stickableLeft && (pushPixels[slimeY] == null || pushPixels[slimeY][slimeX] == null)) {
                            queue.push(slimeX + slimeY * gridWidth + (pixels[grid[(slimeX + slimeY * gridWidth) * gridStride + ID]].sticky && stronglyConnected ? (gridWidth * gridHeight) : 0));
                        }
                    }
                }
...
function pushRight(x, y, strength) {
    pushStrength = strength;
    let [worked, pushPixels, hasCollapsable] = pushRightCheck(x, y, 0);
    if (worked) {
        for (let i in pushPixels) {
            for (let j in pushPixels[i]) {
                if (pushPixels[i][j] != 2 && pushPixels[i][j] != 1) {
        ...
        if (pushPixels[pushY] == null || pushPixels[pushY][pushX] == null) {
            for (let i in pushPixels) {
                for (let j in pushPixels[i]) {
                    ...
                    i = Number(i);
                    let j2 = gridWidth - Number(j);
                    if (failedPushPixels[j2 + i * gridWidth] == tick) {
                        modal("Push error", "Failed push pixel setter right", "error");
                    }
                    failedPushPixels[j2 + i * gridWidth] = tick;
                    // addPixel(j, i, SAND)
                }
            }
        }
        if (hasCollapsable) {
            [worked, pushPixels, hasCollapsable] = pushRightCheck(x, y, 1);
            if (worked) {
                for (let i in pushPixels) {
                    for (let j in pushPixels[i]) {
                        if (pushPixels[i][j] != 2 && pushPixels[i][j] != 1) {
...
function pushRightCheck(x, y, canCollapse) {
    setPushPixels(canCollapse);
    let pushPixels = [];
    let queue = [x + y * gridWidth + gridWidth * gridHeight];
    let worked = true;
        ...
        for (x2 = x1; x2 < gridWidth; x2++) {
            ...
            if (id == AIR) {
                if (pushStrength == 0 && x2 != gridWidth - 1) {
                    let index2 = (x2 + 1 + y1 * gridWidth) * gridStride;
                    let id1 = grid[index2 + ID];
                    if (id1 == PUSHER_LEFT && !isDeactivated(x2 + 1, y1)) {
                        pushFail(x2);
                        break push;
                    }
                }
            ...
            if (id == COLLAPSABLE && x2 != x1) {
                ...
                if (canCollapse != 0) {
                    ...
                    // else if (workedPushPixels[x2 + y1 * gridWidth + gridWidth * gridHeight] != tick) {
                        ...
                        // see if we can push with collapsing
                        // if we cannot, then we collapse this one
                        let [worked1, pushPixels1] = pushRightCheck(x2, y1, 2, pushStrength);
                        if (worked1) {
                            for (let i in pushPixels1) {
                                for (let j in pushPixels1[i]) {
                                    ...
                                    i = Number(i);
                                    let j2 = gridWidth - Number(j);
                                    // workedPushPixels[j2 + i * gridWidth + gridWidth * gridHeight] = tick;
                                    workedPushPixels[j2 + i * gridWidth] = tick;
                                }
                            }
                        }
                            ...
                            if (pushPixels1[pushY] == null || pushPixels1[pushY][pushX] == null) {
                                for (let i in pushPixels1) {
                                    for (let j in pushPixels1[i]) {
                                        ...
                                        i = Number(i);
                                        let j2 = gridWidth - Number(j);
                                        // if (failedPushPixels[j2 + i * gridWidth + gridWidth * gridHeight] == tick) {
                                        //     modal("Push error", "Collapsable failed push pixel setter right", "error");
                                        // }
                                        // failedPushPixels[j2 + i * gridWidth + gridWidth * gridHeight] = tick;
                                        if (failedPushPixels[j2 + i * gridWidth] == tick) {
                                            modal("Push error", "Collapsable failed push pixel setter right", "error");
                                        }
                                        failedPushPixels[j2 + i * gridWidth] = tick;
            ...
            if (canCollapse == 0) {
                ...
                if (workedPushPixels[x2 + y1 * gridWidth] == tick) {
                    if (grid[index1 + UPDATED] != tick) {
                        modal("Push error", "ERROR BUH BUH BUH MASSIVE BUG", "error");
                    }
                    if (x2 != x1) {
                        pushFail(x2 + 1);
                        modal("Push error", "Tried to push already pushed pixel right", "error");
                        break push;
                    }
                    continue push;
                }
        ...
        for (let x2 = x1; x2 < xPos; x2++) {
            ...
            if (pixels[id].sticky) {
                if (y1 > 0) {
                    let slimeX = x2;
                    let slimeY = y1 - 1;
                    if (pixels[grid[(slimeX + slimeY * gridWidth) * gridStride + ID]].stickableDown && (pushPixels[slimeY] == null || pushPixels[slimeY][gridWidth - slimeX] == null)) {
                        queue.push(slimeX + slimeY * gridWidth + (pixels[grid[(slimeX + slimeY * gridWidth) * gridStride + ID]].sticky && stronglyConnected ? (gridWidth * gridHeight) : 0));
                    }
                }
                if (y1 < gridHeight - 1) {
                    let slimeX = x2;
                    let slimeY = y1 + 1;
                    if (pixels[grid[(slimeX + slimeY * gridWidth) * gridStride + ID]].stickableUp && (pushPixels[slimeY] == null || pushPixels[slimeY][gridWidth - slimeX] == null)) {
                        queue.push(slimeX + slimeY * gridWidth + (pixels[grid[(slimeX + slimeY * gridWidth) * gridStride + ID]].sticky && stronglyConnected ? (gridWidth * gridHeight) : 0));
                    }
                }
                if (x2 == x1) {
                    if (x2 > 0) {
                        let slimeX = x2 - 1;
                        let slimeY = y1;
                        if (pixels[grid[(slimeX + slimeY * gridWidth) * gridStride + ID]].stickableLeft && (pushPixels[slimeY] == null || pushPixels[slimeY][gridWidth - slimeX] == null)) {
                            queue.push(slimeX + slimeY * gridWidth + (pixels[grid[(slimeX + slimeY * gridWidth) * gridStride + ID]].sticky && stronglyConnected ? (gridWidth * gridHeight) : 0));
                        }
                    }
                }
...
function pushUp(x, y, strength) {
    pushStrength = strength;
    let [worked, pushPixels, hasCollapsable] = pushUpCheck(x, y, 0);
    if (worked) {
        for (let i in pushPixels) {
            for (let j in pushPixels[i]) {
                if (pushPixels[i][j] != 2 && pushPixels[i][j] != 1) {
            ...
            for (let i in pushPixels) {
                for (let j in pushPixels[i]) {
                    ...
                    if (failedPushPixels[j + i * gridWidth] == tick) {
                        modal("Push error", "Failed push pixel setter up", "error");
                    }
        ...
        if (hasCollapsable) {
            [worked, pushPixels, hasCollapsable] = pushUpCheck(x, y, 1);
            if (worked) {
                for (let i in pushPixels) {
                    for (let j in pushPixels[i]) {
                        if (pushPixels[i][j] != 2 && pushPixels[i][j] != 1) {
...
function pushUpCheck(x, y, canCollapse) {
    setPushPixels(canCollapse);
    let pushPixels = [];
    let queue = [x + y * gridWidth + gridWidth * gridHeight];
    let worked = true;
        ...
        for (y2 = y1; y2 >= 0; y2--) {
            ...
            if (id == AIR) {
                if (pushStrength == 0 && y2 != 0) {
                    ...
                    let id1 = grid[index2 + ID];
                    if (id1 == PUSHER_DOWN && !isDeactivated(x1, y2 - 1)) {
                        pushFail(y2);
                        break push;
                    }
                }
            ...
            if (id == COLLAPSABLE && y2 != y1) {
                ...
                if (canCollapse != 0) {
                    ...
                    // if (failedPushPixels[x1 + y2 * gridWidth + gridWidth * gridHeight] == tick) {
                    if (failedPushPixels[x1 + y2 * gridWidth] == tick) {
                        yPos = y2;
                        break;
                    }
                    // else if (workedPushPixels[x1 + y2 * gridWidth + gridWidth * gridHeight] != tick) {
                    else if (workedPushPixels[x1 + y2 * gridWidth] != tick) {
                        // pushLeft(x2, y1);
                        // // tryPushLeft(x2, y1, pushPistons);

                        // see if we can push with collapsing
                        // if we cannot, then we collapse this one
                        let [worked1, pushPixels1] = pushUpCheck(x1, y2, 2, pushStrength);
                        if (worked1) {
                            for (let i in pushPixels1) {
                                for (let j in pushPixels1[i]) {
                                    if (pushPixels1[i][j] != 2 && pushPixels1[i][j] != 1) {
                            ...
                            if (pushPixels1[pushY] == null || pushPixels1[pushY][pushX] == null) {
                                for (let i in pushPixels1) {
                                    for (let j in pushPixels1[i]) {
                                        ...
                                        if (failedPushPixels[j + i * gridWidth] == tick) {
                                            modal("Push error", "Collapsable failed push pixel setter up", "error");
                                        }
            ...
            // if (failedPushPixels[x1 + y2 * gridWidth + (canCollapse ? (gridWidth * gridHeight) : 0)] == tick) {
            if (failedPushPixels[x1 + y2 * gridWidth] == tick) {
                if (y2 != y1) {
                    pushFail(y2 + 1);
                }
            ...
            if (canCollapse == 0) {
                // if (workedPushPixels[x1 + y2 * gridWidth + (canCollapse ? (gridWidth * gridHeight) : 0)] == tick) {
                if (workedPushPixels[x1 + y2 * gridWidth] == tick) {
                    if (y2 != y1) {
                        pushFail(y2 + 1);
                        modal("Push error", "Tried to push already pushed pixel up", "error");
                        break push;
                    }
                    continue push;
                }
            ...
            if (pushPixels[y2][x1] == null) {
                if (pushStrength < 2 && id == PISTON_DOWN && !isDeactivated(x2, y1)) {
                    pushFail(y2);
                    break push;
                }
        ...
        for (let y2 = y1; y2 > yPos; y2--) {
            ...
            if (pixels[id].sticky) {
                if (x1 > 0) {
                    let slimeX = x1 - 1;
                    let slimeY = y2;
                    if (pixels[grid[(slimeX + slimeY * gridWidth) * gridStride + ID]].stickableRight && (pushPixels[slimeY] == null || pushPixels[slimeY][slimeX] == null)) {
                        queue.push(slimeX + slimeY * gridWidth + (pixels[grid[(slimeX + slimeY * gridWidth) * gridStride + ID]].sticky && stronglyConnected ? (gridWidth * gridHeight) : 0));
                    }
                }
                if (x1 < gridWidth - 1) {
...
function pushDown(x, y, strength) {
    pushStrength = strength;
    let [worked, pushPixels, hasCollapsable] = pushDownCheck(x, y, 0);
    if (worked) {
        for (let i in pushPixels) {
            for (let j in pushPixels[i]) {
                if (pushPixels[i][j] != 2 && pushPixels[i][j] != 1) {
        ...
        if (pushPixels[pushY] == null || pushPixels[pushY][pushX] == null) {
            for (let i in pushPixels) {
                for (let j in pushPixels[i]) {
                    ...
                    let i2 = gridHeight - Number(i);
                    j = Number(j);
                    if (failedPushPixels[j + i2 * gridWidth] == tick) {
                        modal("Push error", "Failed push pixel setter down", "error");
                    }
                    failedPushPixels[j + i2 * gridWidth] = tick;
                    // addPixel(j, i, SAND)
                }
            }
        }
        if (hasCollapsable) {
            [worked, pushPixels, hasCollapsable] = pushDownCheck(x, y, 1);
            if (worked) {
                for (let i in pushPixels) {
                    for (let j in pushPixels[i]) {
                        if (pushPixels[i][j] != 2 && pushPixels[i][j] != 1) {
...
function pushDownCheck(x, y, canCollapse) {
    setPushPixels(canCollapse);
    let pushPixels = [];
    let queue = [x + y * gridWidth + gridWidth * gridHeight];
    let worked = true;
        ...
        for (y2 = y1; y2 < gridHeight; y2++) {
            ...
            if (id == AIR) {
                if (pushStrength == 0 && y2 != gridHeight - 1) {
                    let index2 = (x1 + (y2 + 1) * gridWidth) * gridStride;
                    let id1 = grid[index2 + ID];
                    if (id1 == PUSHER_UP && !isDeactivated(x1, y2 - 1)) {
                        pushFail(y2);
                        break push;
                    }
                }
            ...
            if (id == COLLAPSABLE && y2 != y1) {
                ...
                if (canCollapse != 0) {
                    ...
                    // else if (workedPushPixels[x1 + y2 * gridWidth + gridWidth * gridHeight] != tick) {
                        ...
                        // see if we can push with collapsing
                        // if we cannot, then we collapse this one
                        let [worked1, pushPixels1] = pushDownCheck(x1, y2, 2);
                        if (worked1) {
                            for (let i in pushPixels1) {
                                for (let j in pushPixels1[i]) {
                                    if (pushPixels1[i][j] != 2 && pushPixels1[i][j] != 1) {
                                    ...
                                    let i2 = gridHeight - Number(i);
                                    j = Number(j);
                                    // workedPushPixels[j + i2 * gridWidth + gridWidth * gridHeight] = tick;
                                    workedPushPixels[j + i2 * gridWidth] = tick;
                                }
                            }
                        }
                            ...
                            if (pushPixels1[pushY] == null || pushPixels1[pushY][pushX] == null) {
                                for (let i in pushPixels1) {
                                    for (let j in pushPixels1[i]) {
                                        ...
                                        let i2 = gridHeight - Number(i);
                                        j = Number(j);
                                        // if (failedPushPixels[j + i2 * gridWidth + gridWidth * gridHeight] == tick) {
                                        //     modal("Push error", "Collapsable failed push pixel setter down", "error");
                                        // }
                                        // failedPushPixels[j + i2 * gridWidth + gridWidth * gridHeight] = tick;
                                        if (failedPushPixels[j + i2 * gridWidth] == tick) {
                                            modal("Push error", "Collapsable failed push pixel setter down", "error");
                                        }
                                        failedPushPixels[j + i2 * gridWidth] = tick;
            ...
            if (canCollapse == 0) {
                ...
                if (workedPushPixels[x1 + y2 * gridWidth] == tick) {
                    if (y2 != y1) {
                        pushFail(y2 + 1);
                        modal("Push error", "Tried to push already pushed pixel down", "error");
                        break push;
                    }
                    continue push;
                }
            ...
            if (pushPixels[gridHeight - y2][x1] == null) {
                if (pushStrength < 2 && id == PISTON_UP && !isDeactivated(x2, y1)) {
                    pushFail(y2);
                    break push;
                }
        ...
        for (let y2 = y1; y2 < yPos; y2++) {
            ...
            if (pixels[id].sticky) {
                if (x1 > 0) {
                    let slimeX = x1 - 1;
                    let slimeY = y2;
                    if (grid[(slimeX + slimeY * gridWidth) * gridStride + ID] != AIR && (pushPixels[gridHeight - slimeY] == null || pushPixels[gridHeight - slimeY][slimeX] == null)) {
                        queue.push(slimeX + slimeY * gridWidth + (pixels[grid[(slimeX + slimeY * gridWidth) * gridStride + ID]].sticky && stronglyConnected ? (gridWidth * gridHeight) : 0));
                    }
                }
                if (x1 < gridWidth - 1) {
